...
public class TweakedLinkedControllerClientHandler
{
    ...
    protected static void onReset()
    {
        ...
        if (inLectern())
            CatnipServices.NETWORK.sendToServer(new TweakedLinkedControllerStopLecternPacket(lecternPos));
        lecternPos = null;

        if (buttonStates != 0)
        {
            buttonStates = 0;
            CatnipServices.NETWORK.sendToServer(new TweakedLinkedControllerButtonPacket(buttonStates));
        }
        axisStates = 0;
        CatnipServices.NETWORK.sendToServer(new TweakedLinkedControllerAxisPacket(axisStates, null));
    ...
    public static void tick()
    {
        ...
        if (MODE == Mode.ACTIVE)
        {
            ...
            if (pressedKeys != buttonStates)
            {
                ...
                CatnipServices.NETWORK.sendToServer(new TweakedLinkedControllerButtonPacket(pressedKeys, lecternPos));
            ...
            if (useFullPrec)
            {
                CatnipServices.NETWORK.sendToServer(new TweakedLinkedControllerAxisPacket(TweakedControlsUtil.output.fullAxis, axis, lecternPos));
            }
            else
            {
                if (axis != axisStates)
                {
                    CatnipServices.NETWORK.sendToServer(new TweakedLinkedControllerAxisPacket(axis, lecternPos));
        ...
        if (MODE == Mode.BIND)
        {
            ...
            for (int i = 0; i < 15; i++)
            {
                ...
                {
                    CatnipServices.NETWORK.sendToServer(new TweakedLinkedControllerBindPacket(i, selectedLocation));
            ...
            for (int i = 0; i < 6; i++)
            {
                if ((i < 4 && Math.abs(GamepadInputs.axis[i]) > 0.8f) || (i >= 4 && GamepadInputs.axis[i] > 0))
                {
                    ...
                    if (linkBehaviour != null)
                    {
                        ...
                        CatnipServices.NETWORK.sendToServer(new TweakedLinkedControllerBindPacket(a + 15, selectedLocation));
